...
/**
 * INTMetaItem is item that can have up to Short.MAX_VALUE items inside one id. These items even can be edible, have custom
 ...
 * <p>
 * Items are added in INTMetaItem via {@link #addItem(int, String)}. You will get {@link MetaValueItem} instance, which you
...
public abstract class INTMetaItem<T extends INTMetaItem<?>.MetaValueItem> extends Item
    ...
    public ResourceLocation createItemModelPath(T metaValueItem, String postfix) {
        return INTUtils.INTID(formatModelPath(metaValueItem) + postfix);
    }

    protected String formatModelPath(T metaValueItem) {
        // 将.替换为斜杠
        String path = metaValueItem.unlocalizedName.replaceAll("\\.", "/");
        // 移除开头和结尾的斜杠（如果有的话）
        path = path.replaceAll("^/", "").replaceAll("/$", "");
        return "metaitems/" + path;
    }
    ...
    public final T addItem(int metaValue, String unlocalizedName) {
        Validate.inclusiveBetween(0, Short.MAX_VALUE - 1, metaValue + metaItemOffset,
                "INTMetaItem ID should be in range from 0 to Short.MAX_VALUE-1");
    ...
    public void addInformation(@NotNull ItemStack itemStack, @Nullable World worldIn, @NotNull List<String> lines,
        ...
        if (fluidHandler != null) {
            ...
            if (fluidHandler instanceof IFilteredFluidContainer) {
                IFilteredFluidContainer filtered = (IFilteredFluidContainer) fluidHandler;
                if (filtered.getFilter() instanceof IPropertyFluidFilter) {
                    IPropertyFluidFilter propertyFilter = (IPropertyFluidFilter) filtered.getFilter();
        ...
        if (ConfigHolder.misc.debug) {
            lines.add("INTMetaItem Id: " + item.unlocalizedName);
        }
    }
